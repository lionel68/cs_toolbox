...
dat_spat <- read.csv("C01_spatial_data.csv")
dat_pred <- read.csv("C01_spatial_data_prediction.csv")
...
## the color scale
lprange <- range(dat_pred$p_pred, dat_pred$real_p_occ)
colsc <- scale_fill_gradientn(colours = RColorBrewer::brewer.pal(9, "YlOrRd"), limits = lprange)
